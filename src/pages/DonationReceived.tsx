...
const CareHomeDonationsPage: React.FC = () => {
  ...
  const [selectedDonation, setSelectedDonation] = useState<ExtendedDonation | null>(null);
    ...
    const fetchDonations = async () => {
      try {
        ...
        const typedDonations: ExtendedDonation[] = data.map(d => ({
          ...d,
          status: d.status === "completed" || d.status === "rejected" 
            ? d.status 
            : "pending"
        }));
  ...
  const filteredData = donations.filter(donation => 
    donation.need?.itemName.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const handleStatusChange = (donation: ExtendedDonation, status: "completed" | "rejected") => {
              ...
              columns={[
                { 
                  ...
                  accessor: (item: ExtendedDonation) => (
                    <div>
                      <div className="font-medium">{item.donor?.name || "Unknown"}</div>
                ...
                { 
                  ...
                  accessor: (item: ExtendedDonation) => (
                    <div>
                      ...
                      <div className="text-sm text-gray-500">{item.need?.category}</div>
                    </div>
                  ) 
                },
                { 
        ...
        <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex justify-center items-center">
          <div className="bg-white p-6 rounded-md shadow-lg">
            ...
            <p>
              ...
              <strong>{selectedDonation.quantity} {selectedDonation.need?.itemName}</strong> as{" "}
